...
def single_model_training(config_file=None):
    ...
    # set metadata and metadata-mapping config
    if 'metadata_file' not in config_args:
        print('metadata_file not defined, trying default (overwrite in in src/config.yaml -> defaults)')
        try:
            metadata_file = config['defaults'].get('metadata_path', 'data/processed/221111_encode_metadata_processed.tsv')
        except KeyError as e:
            metadata_file = 'data/processed/221111_encode_metadata_processed.tsv'
        assert os.path.isfile(metadata_file)
        config_args['metadata_file'] = metadata_file
    if 'metadata_mapping_config' not in config_args:
        print('metadata_mapping_config not defined, trying default (overwrite in in src/config.yaml -> defaults)')
        try:
            metadata_mapping_config = config['defaults'].get('metadata_mapping_config', 'src/config_metadata_target_interactions.yaml')
        except KeyError as e:
            metadata_mapping_config = 'src/config_metadata_target_interactions.yaml'
        assert os.path.isfile(metadata_mapping_config)
        config_args['metadata_mapping_config'] = metadata_mapping_config
    
    metadata_loader = dataloading.metadata.MetadataLoader(
        label_ids=datamodule.data.labelloader.label_ids,
        **config_args,
    )

    model_cls = config_args.get('model_class', 'IEAquaticDilated') # get the model class or the default (IEAquaticDilated)
    model_cls = getattr(models, model_cls)
    model = model_cls(n_classes=datamodule.n_classes, metadata_loader=metadata_loader, **config_args)
